...
class AddPlanFields extends StatefulWidget {
  const AddPlanFields(
      {super.key, required this.onpress, this.edit = false, this.plan});
...
class _AddPlanFieldsState extends State<AddPlanFields> {
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<ManagementController>(builder: (managementctrl) {
      return SingleChildScrollView(
        child: Padding(
          padding: EdgeInsets.all(
              MediaQuery.sizeOf(context).width <= mobilescreen ? 16.0 : 32),
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  IconButton(
                      onPressed: widget.onpress, icon: const Icon(Icons.close))
                ],
              ),
              const SizedBox(
                height: 20,
              ...
              Form(
                ...
                child: Column(
                  children: [
                    TextFieldWidget(
                        hint: "Plan Name", controller: _plannamecontroller),
                    const SizedBox(
                      height: 10,
                    ),
                    TextFieldWidget(
                    ...
                    TextFieldWidget(
                      hint: "Discount",
                      controller: _plandiscountcontroller,
                      validator: () {
                        return numberauth(_plandiscountcontroller.text);
                      },
                    ),
                    const SizedBox(
                      height: 10,
                          ...
                          onSelected: (index) {
                            setState(() {
                              _plancat = plancategory[
                                  (index ?? 0) % plancategory.length];
                            });
                          },
                          hintText: _plancat,
                          dropdownMenuEntries: plancategory
                              .asMap()
                              .entries
